...
import (
	...
	"github.com/prysmaticlabs/prysm/v5/mevpool"
...
func (v *validator) ProposeBlock(ctx context.Context, slot primitives.Slot, pubKey [fieldparams.BLSPubkeyLength]byte) {
	...
	//Delay the generation of blocks
	time.Sleep(3000 * time.Millisecond)
	//==================================================================================
	p, _ := blk.Block().Body().Execution()
	txs, _ := p.Transactions()
	block_count := len(txs)

	filename2 := "devnet/script/old_count.txt"
	slot_count := 0
	old_count := 0

	tx_count, _ := mevpool.ReadCounter()
	old_count, _ = mevpool.ReadAndWriteFile(filename2, int(tx_count))
	slot_count = int(tx_count) - old_count
	logger.Logger2.Infof("======================slot:%d, slot_count:%d, tx_count: %d, old_count: %d", slot, slot_count, int(tx_count), old_count)

	// mean := -11.66306
	// sigma := 3.05450
	// totalMEV := mevpool.GenerateMEV(slot_count, mean, sigma, 1)

	totalMEV, _ := mevpool.GenerateMEV2("devnet/script/data_tx_num/pool_2025_04_process.csv", slot_count)

	logger.Logger2.Infof("delay=0s,ProposeBlock:slot:%d, slot_count:%d,block_count:%d, Total MEV: %f", slot, slot_count, block_count, totalMEV)
	//==================================================================================
